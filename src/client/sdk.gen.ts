...
import type {
  ...
  ApplyWorkflowPlanSkeletonData,
  ApplyWorkflowPlanSkeletonResponses,
  ApplyWorkflowPlanSkeletonErrors,
 ...
 * Apply AI workflow edit changes for a given session. Takes a session ID and applies the workflow skeleton structure generated by the AI. This creates the workflow structure and empty component placeholders. Returns component generation instructions that must be used with the generate-component endpoint to flesh out each component with its actual implementation.
 */
export const applyWorkflowPlanSkeleton = <ThrowOnError extends boolean = false>(
  options: Options<ApplyWorkflowPlanSkeletonData, ThrowOnError>,
) => {
  return (options.client ?? client).post<
    ApplyWorkflowPlanSkeletonResponses,
    ApplyWorkflowPlanSkeletonErrors,
