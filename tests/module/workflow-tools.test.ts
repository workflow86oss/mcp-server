...
describe('Workflow Tools Module Tests', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      const result = await callTool('list-workflows', {pageNumber:0, status: 'ALL'});
      ...
      if (result.content?.[0]?.type === 'text') {
        ...
        if (!isPlainTextResponse(response)) {
          ...
          if (workflows.length > 0) {
            publishedWorkflowId = workflows.find(x => x.published)?.workflowId;
            draftWorkflowId = workflows.find(x => !x.published)?.workflowId;
          }
        }
      }
      ...
      throw new Error(`Could not fetch test workflows: ${(error as Error).message}`);
    }
  }, 5000);
  ...
  describe('list-workflows', () => {
    it('should return paginated workflow summaries', async () => {
      const result = await callTool(
        'list-workflows', { status: 'ALL', pageNumber: 0 });
    ...
    it('should filter by publication status', async () => {
      const allResult = await callTool('list-workflows',
         { status: 'ALL', pageNumber: 0 });

      const publishedResult = await callTool('list-workflows',
        { status: 'PUBLISHED', pageNumber: 0 },
      );
  ...
  describe('Integration scenarios', () => {
    ...
    it('should verify workflow state consistency across operations', async () => {
      ...
      if (!initialWorkflow) {
        const availableIds = allWorkflows.map((w: any) => w.workflowId).join(', ');
        throw new Error(`Test workflow not found in first 2 pages. Looking for: ${publishedWorkflowId}. Available workflows across pages 0-1: [${availableIds}]`);
      }
      ...
      const afterPage0Response = parseResponse(afterPage0Result.content![0].text!);
      ...
      if (afterPage0Response['@links']?.nextPage) {
        ...
        const afterPage1Response = parseResponse(afterPage1Result.content![0].text!);
        if (!isPlainTextResponse(afterPage1Response)) {
          afterPublishWorkflows = [...afterPublishWorkflows, ...(afterPage1Response.workflows || [])];
        }
      }
      ...
      if (!publishedWorkflow) {
        const availableIds = afterPublishWorkflows.map((w: any) => w.workflowId).join(', ');
        throw new Error(`Workflow disappeared after publish operation. Looking for: ${publishedWorkflowId}. Available after publish: [${availableIds}]`);
      }
