...
describe("Nested Field Descriptions", () => {
  it("should provide nested field descriptions for WorkflowVersionDetails", () => {
    ...
    ); // This is the actual field from the schema
    expect(components).toHaveProperty(
      "componentId",
  ...
  it("should provide nested field descriptions for SessionResult", () => {
    ...
    expect(componentResults).toHaveProperty(
      "resultId",
  ...
  it("should handle simple properties without nesting", () => {
    ...
    expect((metadata["@schema"] as any).workflowId).toBe(
  ...
  it("should show both array-level and nested descriptions", () => {
    ...
    expect((metadata["@schema"] as any).components.description).toBe(
      "Description of the Component",
    );
    expect((metadata["@schema"] as any).components.componentId).toBe(
      "UUID identifier of the Component",
    );

    // Tables should have nested field descriptions
    expect((metadata["@schema"] as any).tables.name).toBe(
